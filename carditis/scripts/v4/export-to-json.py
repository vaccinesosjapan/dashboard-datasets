...
def main():
	...
	df = pd.read_csv(csv_file_path, delimiter=',', dtype={'lot_no': str})
	...
	# PT名が改行区切りになっている想定で、文字列配列化する
	split_PT_names_series = df['PT_names'].str.replace('\r\n', '\n').str.split('\n')
	...
	df.loc[:, 'vaccine_name'] = df['vaccine_name'].map(lambda x: unicodedata.normalize("NFKC", str(x)))
	...
	# ソースの情報を一覧に追加する
	df.insert(df.shape[1], 'source', [{ "name": source_name, "url": source_url }])
