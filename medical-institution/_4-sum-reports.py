...
for file in jsonFileList:
	with open(file, "r", encoding='utf-8') as f:
		...
		for d in data:
			...
			# 2025年以降は「男性」や「女性」ではなく「男」や「女」と表記されるようになったので、そちらに統一する
			d['gender'] = d['gender'].replace('男性', '男').replace('女性', '女')
			# 同時接種の情報を別要素にしたので、単純な改行除去と大文字小文字の正規化だけで良くなった。
			d['vaccine_name'] = unicodedata.normalize("NFKC", d['vaccine_name'].replace('\r\n', '\n').replace('\n', ''))
			d['manufacturer'] = unicodedata.normalize("NFKC", d['manufacturer'].replace('\r\n', '\n').replace('\n', ''))
